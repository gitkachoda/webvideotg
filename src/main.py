...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):  # pylint: disable=unused-argument
    ...
    # # Send checkpoint: Video downloaded
    # await update.message.reply_text(f"✅ Video downloaded successfully! File path: {video_path}")
    ...
    # Send checkpoint: Video compression (if any)
    # await update.message.reply_text(f"✅ Video compression complete.")
